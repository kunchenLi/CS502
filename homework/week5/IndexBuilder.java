...
public class IndexBuilder {
	...
	public Boolean buildPositionalIndex(Ad ad)
	{
		String keyWords = Utility.strJoin(ad.keyWords, ",");
		List<String> tokens = Utility.cleanedTokenize(keyWords);
		for(int i = 0; i < tokens.size();i++)
		{
			String key = tokens.get(i);
			if(cache.get(key) instanceof Map)
			{
				@SuppressWarnings("unchecked")
				Map<Long, List<Long>> adList = (Map<Long, List<Long>>)cache.get(key);
				adList.get(ad.adId).add((long) i);
				cache.set(key, EXP, adList);
			}
			else
			{
				Map<Long, List<Long>>  adList = new HashMap<>();
				adList.get(ad.adId).add((long) i);
				cache.set(key, EXP, adList);
			}
		}
